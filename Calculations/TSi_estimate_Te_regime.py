...
def test1():
    ...
    attempt_Te, attempt_regime = estimate_Te_and_regime(
        delta_lambda_m = 5.57e-10,   # example Δλ in meters
    ...
    assert abs(attempt_Te - expect_Te) < 1e-6, \
        f"Fail Te: got {attempt_Te}, expected {expect_Te}"
    assert attempt_regime == expect_regime, \
        f"Fail regime: got {attempt_regime}, expected {expect_regime}"
    print("Pass")
...
def test2():
    ...
    attempt_Te, attempt_regime = estimate_Te_and_regime(
        ...
        lambda_i_m     = 532e-9,    # probe wavelength in meters
        theta_deg      = 90.0,      # scattering angle
        ne_m3          = 1.0e26,    # electron density in m^-3
        Z              = 1.0,       # mean ion charge
        m_i_kg         = 1.6726219e-27,  # proton mass
        Ti_over_Te     = 0.1
    )
...
def test3():
    ...
    attempt_Te, attempt_regime = estimate_Te_and_regime(
        ...
        Z              = 6.0,       # mean ion charge
        m_i_kg         = 12*1.6726219e-27,  # proton mass
        Ti_over_Te     = 0.1
    )
