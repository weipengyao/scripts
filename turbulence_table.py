...
# Gekko shock 2024 Ciardi
# V = 3*1e7; B = 0.01; T_e = 80.; T_i = 45.; n_e = 1.55e19; space_scale = 0.03; A = 14.0; Z = 6
...
Ther_equilib_ie_cou_Brag = 1.0 / (
    1.8e-19
    * n_e
    * cou_log_brag
    * Z**2
    * (mass_electron * m_i) ** (1.0 / 2)
    / (mass_electron * T_i + m_i * T_e) ** (3.0 / 2)
)
Ther_equilib_ei_cou_Brag = 1.0 / (
    1.8e-19
    * n_i
    * cou_log_brag
    * Z**2
...
c_sound = np.sqrt(
    5.0 * (n_i * T_i * K_Bol * 1.16e4 + n_e * T_e * K_Bol * 1.16e4) / (3.0 * rho)
)  # P_ther*gamma = rho*V_s^2 (1 eV = 1.16e4 K)
print("Sound velocity            = {:.1e}".format(c_sound), "[cm/s]")
c_alfven = B / (4 * np.pi * n_i * m_i) ** (1.0 / 2)  # Formula from NRL p.29
...
f_e = (
    2.91e-6 * Z * (n_e) * cou_log_brag * (T_e) ** (-3.0 / 2)
)  # [s-1]  Formula from NRL p.28
tau_e = 1.0 / f_e  # [s]
# thermal electron velocity
vth_e = 4.19e7 * (T_e) ** (1.0 / 2)  # [cm/sec] Formula from NRL p.29
...
print("Electron collision time   = {:.2e}".format(tau_e * 1e9), "[ns]")
...
### ion collision time
f_i = (
    4.8e-8 * Z**4 * (A) ** (-1.0 / 2) * n_i * cou_log_brag * (T_i) ** (-3.0 / 2)
)  # [s-1]  Formula from NRL p.28
tau_i = 1.0 / f_i  # [s]
# thermal ion velocity
vth_i = 9.79e5 * (A) ** (-1.0 / 2) * (T_i) ** (1.0 / 2)  # [cm/s] Formula from NRL p.29
### ion mean free path
...
print("Ion collision time        = {:.2e}".format(tau_i * 1e9), "[ns]")
...
gyrofreq_e = vth_e / gyro_e / (2.0 * np.pi)  # [s-1]
print("Electron gyrofrequency    = {:.2e}".format(gyrofreq_e), "[s-1]")
print("Electron Larmor period    = {:.1e}".format(1.0 / gyrofreq_e), "[s]")
...
### Cooling time
cool_rate = 1.7 * 10e-25 * (Z**2) * T_e ** (1.0 / 2)  # normalized cooling rate
...
print(
    "Beta thermic              = {:.1e}".format(beta_ther_CGS)
)  # [P_ther/(B^2/(8*pi))]
beta_dyn_CGS = rho * V**2.0 / (B**2.0 / (8 * np.pi))
print("Beta dynamic              = {:.1e}".format(beta_dyn_CGS))  # [P_dyn/(B^2/(8*pi))]

print(
    "Beta dynamic + thermal    = {:.1e}".format(beta_dyn_CGS + beta_ther_CGS)
)  # [P_dyn/(B^2/(8*pi))]
...
Euler_CGS = (P_dyn / P_ther) ** (1.0 / 2)
print("Euler number              = {:.1e}".format(Euler_CGS))  # [sqrt(P_dyn/P_ther)]

Alfven_CGS = B / (4 * np.pi * P_ther) ** (1.0 / 2)
